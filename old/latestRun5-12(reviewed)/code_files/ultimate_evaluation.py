...
from ped_corridor_env3 import RealisticPedCorridorEnv
from lshaped_evaluation import LShapedCorridorEnv
from tshaped_env import TShapedCorridorEnv
...
class UltimateEvaluator:
    ...
    def __init__(self, model_path: str):
        ...
        try:
            ...
            print(f"⚠ Warning: Could not load with environment: {e1}")
            try:
    ...
    def create_standard_sparse_env(self) -> RealisticPedCorridorEnv:
        """Create standard sparse environment with obstacles blocking the path."""
        env = RealisticPedCorridorEnv(obstacle_density=0.0)
        # Place obstacles DIRECTLY in the path to force navigation and rotation
        obstacles = []
        corridor_center_y = 5.0  # Center of 10m wide corridor
        
        # Place obstacles that FORCE the agent to rotate and navigate
        x_positions = np.linspace(6, 34, 8)  # 8 obstacles across corridor
        np.random.shuffle(x_positions)
        
        for i, x_pos in enumerate(x_positions):
            pattern = i % 4
            if pattern == 0:
                # Block center path - MUST rotate left or right
                obstacles.append([x_pos - 0.1, corridor_center_y - 1.2, 1.4, 2.8])
            elif pattern == 1:
                # Block left-center - force going right
                obstacles.append([x_pos - 0.1, corridor_center_y - 2.8, 1.2, 2.0])
            elif pattern == 2:
                # Block right-center - force going left
                obstacles.append([x_pos - 0.1, corridor_center_y + 0.8, 1.2, 2.0])
            else:
                # Staggered - left then right pattern
                if i % 2 == 0:
                    obstacles.append([x_pos - 0.1, corridor_center_y - 3.2, 1.0, 1.8])
                else:
                    obstacles.append([x_pos - 0.1, corridor_center_y + 1.2, 1.0, 1.8])
        
        env._custom_obstacles = obstacles
        return env
    
    def create_standard_dense_env(self) -> RealisticPedCorridorEnv:
        """Create standard dense environment with obstacles blocking the path."""
        env = RealisticPedCorridorEnv(obstacle_density=0.0)
        # Place obstacles DIRECTLY in the path to force aggressive navigation
        obstacles = []
        corridor_center_y = 5.0  # Center of 10m wide corridor
        
        # Dense obstacles - many blocking the direct path
        x_positions = np.linspace(5, 35, 14)  # 14 obstacles - very dense!
        np.random.shuffle(x_positions)
        
        for i, x_pos in enumerate(x_positions):
            pattern = i % 5
            if pattern == 0:
                # Block center path completely - MUST rotate
                obstacles.append([x_pos - 0.15, corridor_center_y - 1.5, 1.5, 3.2])
            elif pattern == 1:
                # Block left side - force right
                obstacles.append([x_pos - 0.1, corridor_center_y - 3.5, 1.2, 2.2])
            elif pattern == 2:
                # Block right side - force left
                obstacles.append([x_pos - 0.1, corridor_center_y + 1.3, 1.2, 2.2])
            elif pattern == 3:
                # Narrow gap - center-left and center-right
                obstacles.append([x_pos - 0.1, corridor_center_y - 2.8, 1.0, 1.5])
                obstacles.append([x_pos - 0.1, corridor_center_y + 1.8, 1.0, 1.5])
            else:
                # Zigzag pattern
                if i % 2 == 0:
                    obstacles.append([x_pos - 0.1, corridor_center_y - 3.0, 1.1, 2.0])
                else:
                    obstacles.append([x_pos - 0.1, corridor_center_y + 1.0, 1.1, 2.0])
        
        env._custom_obstacles = obstacles
        return env
        
    def create_narrow_passage_env(self) -> RealisticPedCorridorEnv:
        """Create narrow passage environment."""
        env = RealisticPedCorridorEnv(obstacle_density=0.0)
        # Create narrow passages with visible obstacles
        # Store obstacles to preserve after reset
        obstacles = [
            # First narrow section
            [10, 0.5, 2.0, 3.0],   # Bottom wall extension
            [10, 6.5, 2.0, 3.0],   # Top wall extension
            # Second narrow section
            [20, 1.5, 2.0, 2.5],   # Bottom
            [20, 6.0, 2.0, 2.5],   # Top
            # Third narrow section
            [30, 0.5, 2.0, 3.5],   # Bottom
            [30, 6.0, 2.0, 3.5],   # Top
        ]
        # Store obstacles in a way that survives reset
        env._custom_obstacles = obstacles
        return env
    
    def create_zigzag_env(self) -> RealisticPedCorridorEnv:
        """Create zigzag path environment."""
        env = RealisticPedCorridorEnv(obstacle_density=0.0)
        # Alternating wall obstacles - BIGGER and MORE VISIBLE
        obstacles = [
            [8, 0.5, 2.0, 6.0],    # From bottom (bigger)
            [16, 4.0, 2.0, 5.5],   # From top
            [24, 0.5, 2.0, 6.0],   # From bottom
            [32, 4.0, 2.0, 5.5],   # From top
        ]
        # Store obstacles in a way that survives reset
        env._custom_obstacles = obstacles
        return env
    
    def create_clustered_env(self) -> RealisticPedCorridorEnv:
        """Create clustered obstacles environment."""
        env = RealisticPedCorridorEnv(obstacle_density=0.0)
        obstacles = []
        # Cluster 1 (left)
        for i in range(3):
            for j in range(2):
                obstacles.append([10 + i*1.5, 1.5 + j*1.5, 1.2, 1.2])
        # Cluster 2 (center)
        for i in range(2):
            for j in range(3):
                obstacles.append([22 + i*1.5, 2.0 + j*1.5, 1.2, 1.2])
        # Cluster 3 (right)
        for i in range(3):
            for j in range(2):
                obstacles.append([32 + i*1.5, 1.0 + j*1.5, 1.2, 1.2])
        # Store obstacles in a way that survives reset
        env._custom_obstacles = obstacles
        return env
    
    def create_ushaped_env(self, seed: int = 42) -> UltimateDomainRandomizedEnv:
        """Create U-shaped corridor."""
        env = UltimateDomainRandomizedEnv(
            difficulty_level="medium",
            allowed_shapes=["ushaped"]
        )
        env.reset(seed=seed)
        return env
    
    def create_multiroom_env(self, seed: int = 42) -> UltimateDomainRandomizedEnv:
        """Create multi-room corridor."""
        env = UltimateDomainRandomizedEnv(
            difficulty_level="medium",
            allowed_shapes=["multiroom"]
        )
        env.reset(seed=seed)
        return env
    ...
    def save_results(self, output_dir: str = "eval_ultimate"):
        ...
        # Export for 3D visualizer
        try:
            from export_for_3d_visualizer import export_all_episodes
            visualizer_dir = os.path.join(output_dir, "visualizer_data")
            export_all_episodes(f"{output_dir}/complete_results.json", visualizer_dir)
            print(f"✓ 3D visualizer data exported to {visualizer_dir}/")
            print(f"  Open visualizer_3d.html and load any JSON file to visualize in 3D!")
        except ImportError:
            print(f"⚠ 3D visualizer export skipped (export_for_3d_visualizer.py not found)")
        except Exception as e:
            print(f"⚠ 3D visualizer export failed: {e}")
    
    def generate_all_videos(self, output_dir: str = "eval_ultimate"):
        """Generate videos for ALL episodes."""
        print(f"\n📹 Generating videos for ALL {len(self.all_results)} episodes...")
    ...
    def _create_video(self, result: Dict, filename: str):
        ...
        if not trajectory or len(trajectory) == 0:
            print(f"⚠ Skipping video for {filename}: empty trajectory")
            return
        ...
        try:
            ...
            if len(trajectory) == 0:
                print(f"⚠ Skipping video for {filename}: invalid trajectory data")
                return
        except (ValueError, TypeError, IndexError) as e:
            print(f"⚠ Skipping video for {filename}: trajectory parsing error: {e}")
            return
        ...
        for step_idx in range(0, len(trajectory), frame_skip):
            ...
            if corridor_type == "lshaped":
                ...
                if walkable_regions:
                    ...
                    # Handle both old 2-region format and new 3-region format (with overlap)
                    main_regions = []
                    corner_region = None
                    ...
                    # Explicitly fill corner region if it exists
                    if corner_region:
                    ...
                    # Also connect main regions explicitly (for backward compatibility)
                ...
                if walkable_regions:
                    ...
                    for region in walkable_regions:
                        ...
                        corridor = Rectangle((x_min, y_min), x_max - x_min, y_max - y_min,
                                              ...
                                              linewidth=2.5, alpha=0.95, zorder=0)
                        ax.add_patch(corridor)
                    # Connect regions at corners for T-shapes (fix black line issue)
                    # Handle both old 2-region format and new 3-region format (with overlap)
                    main_regions = []
                    ...
                    # Explicitly fill junction region if it exists
                    if junction_region:
                        x_min, x_max, y_min, y_max = junction_region
                        junction_fill = Rectangle((x_min, y_min), x_max - x_min, y_max - y_min,
                                                  facecolor="#F5F5DC", edgecolor="none", 
                                                  zorder=0)
                        ax.add_patch(junction_fill)
