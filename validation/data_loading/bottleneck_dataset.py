...
class BottleneckTrajectory:
    ...
    def get_bottleneck_entry_exit_times(self, bottleneck_x_range: Tuple[float, float]) -> Tuple[float, float]:
        """
        ...
        entry_time = self.timestamps[entry_idx[0]] if len(entry_idx) > 0 else self.timestamps[0]
        
        # Find first time exiting bottleneck
        exit_idx = np.where(x_positions >= bottleneck_x_range[1])[0]
        exit_time = self.timestamps[exit_idx[0]] if len(exit_idx) > 0 else self.timestamps[-1]
...
class BottleneckDatasetLoader:
    ...
    def load_trajectory_hdf5(self, file_path: Path) -> BottleneckTrajectory:
        ...
        with h5py.File(file_path, 'r') as f:
            ...
            gender = f['metadata/gender'][()].decode('utf-8') if 'metadata/gender' in f else None
    ...
    def load_trajectory_txt(self, file_path: Path, metadata: Dict) -> BottleneckTrajectory:
        """
    ...
    def load_all(self) -> List[BottleneckTrajectory]:
        ...
        if self.hdf5_dir.exists():
            hdf5_files = list(self.hdf5_dir.glob("*.h5")) + list(self.hdf5_dir.glob("*.hdf5"))
    ...
    def filter_trajectories(
        ...
        if width is not None:
            filtered = [t for t in filtered if np.isclose(t.bottleneck_width, width, atol=0.01)]
        
        if length is not None:
            filtered = [t for t in filtered if np.isclose(t.bottleneck_length, length, atol=0.01)]
        
        if angle is not None:
            filtered = [t for t in filtered if np.isclose(t.approach_angle, angle, atol=1.0)]
