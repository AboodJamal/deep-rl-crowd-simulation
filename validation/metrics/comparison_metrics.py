...
def compute_avg_point_distance(
    traj1: np.ndarray,
    traj2: np.ndarray
) -> float:
    """
...
def compare_trajectories(
    ...
    # Trajectory similarity
    hausdorff_dist = compute_hausdorff_distance(model_pos_resampled, human_pos_resampled)
    ...
    # Path similarity
    path_overlap_val = compute_path_overlap(model_pos_resampled, human_pos_resampled, corridor_width=0.5)
    ...
    speed_rmse = compute_speed_profile_rmse(
        model_positions, model_velocities,
    ...
    model_progress = 1 - np.linalg.norm(model_pos_resampled - goal_pos, axis=1) / \
                     np.linalg.norm(start_pos - goal_pos)
    human_progress = 1 - np.linalg.norm(human_pos_resampled - goal_pos, axis=1) / \
                     np.linalg.norm(start_pos - goal_pos)
