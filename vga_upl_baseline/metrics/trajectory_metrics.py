...
def compute_lateral_deviation(
    positions: np.ndarray, start_pos: np.ndarray, goal_pos: np.ndarray
...
def compute_speed_profile(positions: np.ndarray, timestamps: np.ndarray) -> np.ndarray:
    """
...
def compute_obstacle_clearances(
    positions: np.ndarray, obstacles: List[Dict], agent_radius: float = 0.225
...
def compute_trajectory_metrics(
    ...
    if N < 2:
        return TrajectoryMetrics(
            ...
            stuck=True,
            goal_distance_final=(
                np.linalg.norm(positions[-1] - goal_pos) if N > 0 else float("inf")
            ),
            comfort_index=0.0,
        )
    ...
    if len(first_progress_idx) > 0:
        time_to_first_progress = float(
            timestamps[first_progress_idx[0]] - timestamps[0]
        )
    else:
    ...
    collision_severity = (
        float(np.sum(np.abs(clearances[collision_mask]))) if num_collisions > 0 else 0.0
    )
