...
class UPLParameters:
    ...
    r: float = 0.15  # Agent radius (m) - human ~0.25-0.3m shoulder width, half = 0.15m
    ...
    max_speed: float = (
        2.5  # Maximum speed limit (m/s) - human sprint ~3m/s, comfortable max ~2.5m/s
    )
...
class UPLPhysics:
    ...
    def calculate_self_propulsion_force(
        self, current_velocity: np.ndarray, desired_direction: np.ndarray
    ) -> np.ndarray:
        """
    ...
    def calculate_total_force(
        ...
        f_self = self.calculate_self_propulsion_force(current_velocity, goal_direction)

        # Interaction forces from all obstacles
        f_int_total = np.zeros(2)
        for obs_pos, obs_radius in obstacles:
            f_int = self.calculate_interaction_force(agent_pos, obs_pos, obs_radius)
            f_int_total += f_int
    ...
    def update_velocity(
        self, current_velocity: np.ndarray, total_force: np.ndarray
    ) -> np.ndarray:
        """
...
def test_upl_physics():
    ...
    min_dist_to_obstacle = min(
        [np.linalg.norm(pos - obstacles[0][0]) for pos in positions]
    )
