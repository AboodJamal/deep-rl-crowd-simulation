...
class TrajectoryVideoGenerator:
    ...
    def __init__(
        self, figsize: Tuple[int, int] = (12, 8), dpi: int = 100, fps: int = 30
    ):
        """
    ...
    def create_single_trajectory_video(
        ...
        # Set up plot - include obstacles in bounds calculation
        all_positions = np.vstack([positions, [start_pos], [goal_pos]])

        # Calculate bounds including obstacles
        x_coords = list(all_positions[:, 0])
        y_coords = list(all_positions[:, 1])

        for obs in obstacles:
            obs_pos = np.array(obs["position"])
            obs_radius = obs.get("radius", 0.3)
            x_coords.extend([obs_pos[0] - obs_radius, obs_pos[0] + obs_radius])
            y_coords.extend([obs_pos[1] - obs_radius, obs_pos[1] + obs_radius])
        ...
        # Draw start and goal
        ax.plot(
            start_pos[0], start_pos[1], "go", markersize=15, label="Start", zorder=3
        )
    ...
    def create_comparison_video(
        ...
        fig, axes = plt.subplots(
            1, num_models, figsize=(6 * num_models, 6), dpi=self.dpi
        )
            ...
            for idx, (model_name, positions) in enumerate(trajectories.items()):
                if frame < len(positions):
                    ...
                    trail_lines[idx].set_data(
                        positions[: frame + 1, 0], positions[: frame + 1, 1]
                    )
    ...
    def create_stochastic_overlay_video(
        ...
        # Set limits - include obstacles in bounds
        all_pos = np.vstack([np.vstack(all_trajectories), [start_pos], [goal_pos]])

        # Include obstacles in bounds calculation
        x_coords = list(all_pos[:, 0])
        y_coords = list(all_pos[:, 1])
        ...
        # Draw start/goal
        ax.plot(
            start_pos[0], start_pos[1], "go", markersize=15, label="Start", zorder=10
        )
        ...
        def animate(frame):
            ...
            count_text.set_text(
                f"Trials shown: {len(all_trajectories)}\\nFrame: {frame}/{max_len}"
            )
...
def generate_videos_from_experiment_results(
    results_json_path: str, output_dir: str, num_stochastic_to_show: int = 20
):
    """
...
if __name__ == "__main__":
    ...
    generate_videos_from_experiment_results(
        results_path, output_dir, num_stochastic_to_show=20
    )
