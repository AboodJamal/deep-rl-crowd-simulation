...
def train_ultimate_curriculum(
    ...
    if total_timesteps <= 0:
        ...
        if os.path.exists(summary_path):
            print(f"\n[SKIP] Skipping training (0 timesteps). Showing previous summary from {summary_path}\n")
            ...
            print("[WARN] No summary file found (curriculum_logs/ultimate_training_summary.json). Nothing to show.")
            return None
    # ------------------------------------------------------------------
    ...
    if latest_stage_num > 0:
        ...
        print(f"[OK] Found most recent checkpoint at stage {resume_from_stage}: {stage_model_path}")
    ...
    for stage_idx, stage in enumerate(stages):
        ...
        if resume_from_stage and (stage_idx + 1) <= resume_from_stage:
            print(f"\n[SKIP] Skipping stage {stage_idx + 1} (already completed)")
            ...
            if stage_idx + 1 == resume_from_stage:
                ...
                try:
                    ...
                    dummy_env.close()
                    print(f"[OK] Loaded model from stage {resume_from_stage}")
                except Exception as e:
                    print(f"[WARN] Could not load model from {stage_model_path}: {e}")
        ...
        print(f"[OK] Stage model saved: {stage_save_path}\n")
    ...
    if vec_norm_final is not None:
        ...
        print(f"[OK] Saved VecNormalize with training statistics from final stage")
        ...
        if os.path.exists(stage6_path):
            ...
            print(f"[OK] Copied stage6 VecNormalize as final VecNormalize")
        else:
            print(f"[WARN] Could not save VecNormalize - use stage6_vecnormalize.pkl manually")
